...
import com.example.erp20.app09.RecyclerItemStockTransOrderHeaderSimplifyAdapter
import com.example.erp20.app09.RecyclerItemStockTransOrderHeaderSimplifyAdapter2
...
class Activity06 : AppCompatActivity() {
    private lateinit var StockTransOrderHeaderSimplify_adapter3: RecyclerItemStockTransOrderHeaderSimplifyAdapter3
        ...
        searchbtn?.setOnClickListener {
            ...
            when(autoCompleteTextView?.text.toString()){
                ...
                "領料申請"->{
                    ...
                    mAlertDialog.setView(item)
                    //filter_combobox選單內容
                    val comboboxView=item.findViewById<AutoCompleteTextView>(R.id.autoCompleteText2)//日期
                    val comboboxView2=item.findViewById<AutoCompleteTextView>(R.id.autoCompleteText)//生產線
                    val arrayAdapter2= ArrayAdapter(this,R.layout.combobox_item,cookie_data.pline_id_name_ComboboxData)
                    comboboxView2.setText(cookie_data.pline_id_name_ComboboxData[0])
                    ...
                    mAlertDialog.setNegativeButton("確定") { dialog, id ->
                        ...
                        when(cookie_data.status)
                        {
                            0->{
                                Toast.makeText(this, "資料載入", Toast.LENGTH_SHORT).show()
                                recyclerView.layoutManager= LinearLayoutManager(this)//設定Linear格式
                                StockTransOrderHeaderSimplify_adapter3= RecyclerItemStockTransOrderHeaderSimplifyAdapter3(selectFilter2,selectFilter)
                                recyclerView.adapter=StockTransOrderHeaderSimplify_adapter3//找對應itemAdapter
                                addbtn?.isEnabled=true
                            }
                            1->{
                                Toast.makeText(this, cookie_data.msg, Toast.LENGTH_SHORT).show()
